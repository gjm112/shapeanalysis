...
start <- Sys.time()
test <- impute_partial_shape(complete_shape_list,partial_shape, M = 50, k = 20)
end <- Sys.time()
end-start

plot(t(test$imputed[[1]]),type="l", col = rgb(0,0,0,0), xlim = c(-0.4, 0.15), ylim = c(-0.2,0.2), lwd = 3)
points(t(red$donor),col = "red", type = "l",lwd=3)
set.seed(1234)
for (i in 1:50){
  u <- runif(3)
  points(t(test$imputed[[i]]),type="l",col =  rgb(u[1],u[2],u[3],0.1), lwd = 3)
}
