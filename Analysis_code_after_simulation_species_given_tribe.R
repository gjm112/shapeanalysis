...
for (tooth in c("LM1","LM2","LM3","UM1","UM2","UM3")){
  for (side in 1:2){print(c(tooth,side))
    ...
    for (knn in c(1:4,6:20,30,40,50,60,5)){print(knn)
      ...
      knn_partial_matching <- function(DSCN, tribe){
        temp <- results_list[[DSCN]]$dist_partial[results_list[[DSCN]]$dist_partial$Tribe == tribe,]
      ...
      tribes <- sort(unique(ref_file$Tribe))
      part_match <- list()
      for (t in 1:length(tribes)){
      part_match[[t]] <- do.call(rbind,lapply(ids, knn_partial_matching, tribe = tribes[t]))
      }
      
      for (t in 1:(length(tribes)-1)){
        nc <- ncol(part_match[[t]])
        part_match[[t]] <- part_match[[t]][-c((nc-1):nc)]
      }
      
      part_match_df <- do.call(cbind,part_match)
      
      names(part_match_df)[21] <- "true"
      names(part_match_df)[22] <- "DSCN"
      ...
      knn_imputed <- function(DSCN, tribe){
        temp <- results_list[[DSCN]]$dist[results_list[[DSCN]]$dist$Tribe == tribe,]
      ...
      imputed_match <- list()
      for (t in 1:length(tribes)){
        imputed_match[[t]] <- do.call(rbind,lapply(ids, knn_imputed, tribe = tribes[t]))
      }
      
      for (t in 1:(length(tribes)-1)){
        nc <- ncol(imputed_match[[t]])
        imputed_match[[t]] <- imputed_match[[t]][-c((nc-1):nc)]
      }
      
      
      imputed_match_df <- do.call(cbind,imputed_match)
      
      names(imputed_match_df)[21] <- "true"
      names(imputed_match_df)[22] <- "DSCN"
    ...
    if (!scaled){
      save(list = c("logloss_imputed","logloss_part","imputed_match_df","part_match_df"), 
           file = paste0("/Users/gregorymatthews/Dropbox/shapeanalysisgit/results/results20190610_side=",side,"_k=",k,"_M=",M,"_tooth=",tooth,"_summaries_species_given_tribe.RData"))
    }
    
    if (scaled){
      save(list = c("logloss_imputed","logloss_part","imputed_match_df","part_match_df"), 
           file = paste0("/Users/gregorymatthews/Dropbox/shapeanalysisgit/results/results20190610_side=",side,"_k=",k,"_M=",M,"_tooth=",tooth,"scaled_summaries_species_given_tribe.RData"))
    }
