...
Out goal is to complete $[\C]_{obs}$ or ($[\C^{\star}]_{obs}$) by randomly drawing from the distribution on the missing part of the shape conditional on the observed part.  Specifically, for a fixed translation, rotation, scaling, and parameterization, we seek to approximate the distribution of $\C_{mis}|\C_{obs}$.  If we  $X_j \sim \C_{mis}|\C_{obs}$ to be a random draw from this distribution, that is a $X_j \in \mathcal{X}_j$ is an open curve that is a plausible completion of the curve $\C_{obs}$ and $\mathcal{X}_j$ is defined as 

\[
\mathcal{X}_j := \{X: \gamma^{-1}(R^{-1}(t)) \rightarrow  \mathbb{R}^2 \}
\]
