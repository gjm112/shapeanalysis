...
Now consider a function, $R$, defined over the set $\mathbb{S}$, where $R(t) = 1$ if the shape function is observed at $t \in \mathbb{S}$ and $R(t) = 0$ otherwise (i.e. the shape function is observaed at $t \in \mathbb{S}$)  
...
Note that for every curve $\C$ and corresponding $R$, in order to maintain the correct relationship, if $\C$ is reparameterized by $\gamma \in \Gamma$
 $[\C]$ there is a correspondo
